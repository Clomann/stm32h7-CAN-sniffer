...
#define LOG_BUFFER_SIZE (8U * BLOCK_SIZE)
...
#define CAN_FLAG_RTR_FDF                                                       \
    (1U << CAN_FLAG_RTR_FDF_Pos) // RTR for classic, FDF for CAN FD
...
{
    ClbEntryTypeType
        type; // e.g., 0 = None, 1 = CAN (FD) frame, 2 = SYNC, 3 = Marker
...
typedef struct __attribute__((packed))
{
    ...
    uint32_t timestamp; /*!< timestamp in us */
    uint32_t
        abs_time_high; /*!< most significant 32 bit of absolute timestamp in us */
} CanLogSyncType;
...
uint8_t CanLogBuffer_ReadNextBlock(
    uint8_t **data,
    uint32_t *len,
