...
__attribute__((at(0x30040000))) ETH_DMADescTypeDef
    DMARxDscrTab[ETH_RX_DESC_CNT]; /* Ethernet Rx DMA Descriptors */
__attribute__((at(0x30040100))) ETH_DMADescTypeDef
    DMATxDscrTab[ETH_TX_DESC_CNT]; /* Ethernet Tx DMA Descriptors */

#elif defined(__GNUC__) /* GNU Compiler */

ETH_DMADescTypeDef
    DMARxDscrTab[ETH_RX_DESC_CNT] ETH_RX_DESC; /* Ethernet Rx DMA Descriptors */
ETH_DMADescTypeDef
    DMATxDscrTab[ETH_TX_DESC_CNT] ETH_TX_DESC; /* Ethernet Tx DMA Descriptors */
...
int32_t
ETH_PHY_IO_ReadReg(uint32_t DevAddr, uint32_t RegAddr, uint32_t *pRegVal);
int32_t
ETH_PHY_IO_WriteReg(uint32_t DevAddr, uint32_t RegAddr, uint32_t RegVal);
...
/* USER CODE BEGIN OS_THREAD_NEW_CMSIS_RTOS_V2 */
    ...
    if (hal_eth_init_status == HAL_OK)
    {
        ...
        else
        {
            ...
            {
                tmp = (DMARxDscrTab[i].DESC0 >> 31) & 0x1;
                tmp = (uint32_t)DMARxDscrTab[i].BackupAddr0;
            }
            netif_set_up(netif);
...
static err_t low_level_output(struct netif *netif, struct pbuf *p)
{
    ...
    for (q = p; q != NULL; q = q->next)
    {
        ...
        AddressAligned_CleanDCache(Txbuffer[i].buffer, Txbuffer[i].len);
...
void send_udp_message(void)
{
    ...
    ip_addr_t dest_ip;
    IP4_ADDR(&dest_ip, 192, 168, 1, 100); // PC or target IP
...
void HAL_ETH_RxAllocateCallback(uint8_t **buff)
{
    ...
    RxBuff_t *rx;
    volatile RxBuff_t *rx2;
    (void)rx2;

    rx = (RxBuff_t *)LWIP_MEMPOOL_ALLOC(RX_POOL);
    if (rx != NULL)
    {
        /* Get the buff from the struct pbuf address. */
        // *buff = (uint8_t *)p + offsetof(RxBuff_t, buff);
        *buff                                = rx->buff; // DMA will write this
        rx->pbuf_custom.custom_free_function = pbuf_free_custom;
...
)
{
    ...
    RxBuff_t *rx = (RxBuff_t *)(buff - offsetof(RxBuff_t, buff));
    p            = (struct pbuf *)&rx->pbuf_custom;
    p->next      = NULL;
    p->tot_len   = 0;
    p->len       = Length;
...
void sys_check_core_locking(void){
    ...
    LWIP_ASSERT(
        "Function called from interrupt context",
...
#warning Below check should be removed in production code
        if (current_thread_id != lwip_core_lock_holder_thread_id)
            __BKPT(0);
