...
static int IpStringToIntArray(const char *ip, uint8_t len, uint8_t *arr)
{
    ...
    for (uint8_t index = 0U; index < len; index++)
    {
        ...
        else if (ip[index] == '.') // Delimiter
        {
            if (CharCount == 0U)
            {
                return ERROR_INVALID_FORMAT; // Prevent ".." or leading/trailing dots
            }

            FileHandler_ConvertToInteger(element, &ValTmp, 10U);
            if (ValTmp > 255)
            {
                return ERROR_INVALID_FORMAT; // Invalid octet value
            }
            ...
            if (ElemCount > 3U)
            {
                return ERROR_TOO_MANY_SEGMENTS; // Too many octets
            }
    ...
    if (CharCount == 0U)
    {
        return ERROR_INVALID_FORMAT; // Handle trailing dot case
    }

    FileHandler_ConvertToInteger(element, &ValTmp, 10U);
    if (ValTmp > 255)
    {
        return ERROR_INVALID_FORMAT; // Check last segment
    }

    arr[ElemCount++] = (uint8_t)ValTmp;

    return (ElemCount == 4U) ? SUCCESS
                             : ERROR_INVALID_FORMAT; // Ensure exactly 4 octets
}
...
int FatFS_SD_LoadConfig(FatFsDeviceType *dev, char *data, uint32_t *len)
{
    ...
    if (fr == FR_OK)
    {
        fr = f_open(
            &dev->file,
            SETTINGSHANDLER_PARTITION_NO "conf.txt",
...
static uint8_t m_CreateJSonString(
    AppConfigType *cfg,
    char *json,
    uint32_t maxLength,
    uint32_t *len
