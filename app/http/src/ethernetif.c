...
static err_t low_level_output(struct netif *netif, struct pbuf *p)
{
  ...
  TxConfig.pData = p;
  memcpy(Buffer, (uint8_t*)TxConfig.TxBuffer[0].buffer, TxConfig.TxBuffer[0].len );
  pbuf_ref(p);
...
/* USER CODE BEGIN HAL ETH RxAllocateCallback */
  ...
  if (rx != NULL)
  {
    ...
    * changed by lwIP or the app, e.g., pbuf_free decrements ref. */
    pbuf_alloced_custom(PBUF_RAW, 0, PBUF_REF, &rx->pbuf_custom, *buff, ETH_RX_BUFFER_SIZE);
    rx2 = rx;
  }
  else
  {
