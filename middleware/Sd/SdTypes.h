...
typedef struct
{
    ...
    uint32_t
        cSize : 22; /*!< [69:48] Device size (memory capacity = (C_SIZE+1) * 512KByte) */
    uint8_t reserved3 : 1;
...
#define HIGH_SPEED_MODE                                                        \
    (0x5A) /*!< SDSC/SDHC/SDXC in High Speed mode  (50MHz) */
#define SDR50_OR_DDR50_MODE                                                    \
    (0x0B) /*!< SDHC/SDXC in SDR50 or DDR50 mode (100Mbit/sec) */
...
#define GET_BYTE_INDEX(bitpos, elemSize) ((bitpos) / elemSize)

#define EXTRACT_CSD_BITS(array, mask, start)                                   \
    (array[GET_BYTE_INDEX(start, 8U)]                                          \
         >> (start - (GET_BYTE_INDEX(start, 8U) * 8U))                         \
     & mask)
