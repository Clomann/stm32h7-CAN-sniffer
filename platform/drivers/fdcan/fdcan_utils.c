...
{
    return ((uint32_t)reg_value[3] << 24) | ((uint32_t)reg_value[2] << 16)
           | ((uint32_t)reg_value[1] << 8) | ((uint32_t)reg_value[0] << 0);
}
...
#define SAMPLE_POINT_FACTOR 10000
    ...
    for (uint8_t tq_num = MaxTq; tq_num >= 8; tq_num--)
    {
        ...
        if (prescaler == 0 || (is_data_phase && prescaler > 32)
            || (!is_data_phase && prescaler > 512))
        {
            continue;
        }
        ...
        if (seg1 > 0 && seg2 > 0)
        {
            ...
            if (is_data_phase)
            {
                reg_value = ((prescaler - 1) << FDCAN_DBTP_DBRP_Pos)
            ...
            {
                reg_value = ((prescaler - 1) << FDCAN_NBTP_NBRP_Pos)
